...
if __name__ == '__main__':
    ...
    true_labels = np.load(path + 'svhn_extra_resnet10_labels_531131_512.npy')
    ...
    dist = "cosine"
    k_max = 20
    path = "/shared/Dataset/Clustering/svhn_resnet10_output/"
    ...
    # n_threads = 8
    # k_max = 50
    # savePath = "/shared/Dataset/Clustering/svhn_resnet50_output/"
    # seed = 42
    # dist="cosine"
    #
    # # NNDescent params
    ...
    #           # init_graph=indices,
    #           max_candidates = 50, # "self-join" size of max 50 points
    ...
    savePath = "/shared/Dataset/Clustering/svhn_resnet10_output/"

    indices = np.load(savePath + f"ivfpq_{nlist}_{nprobe}_{m}_{dist}_{k_max}_indices.npy")    # shape: (n, k), dtype: int32
    distances = np.load(savePath + f"ivfpq_{nlist}_{nprobe}_{m}_{dist}_{k_max}_distances.npy")  # shape: (n, k), dtype: float32
    ...
    n_neighbors_list = [10, 15, 20, 25, 30, 35, 40, 45, 50]
