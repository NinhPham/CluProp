...
if __name__ == '__main__':
    ...
    savePath = Path("~/Work/Datasets/Clustering/pendigit_output").expanduser()

    X = np.loadtxt("test/Dataset/pendigits-data.txt", delimiter=",")
    true_labels = np.loadtxt("test/Dataset/pendigits-labels.txt", delimiter=",")
    ...
    for n_neighbors in n_neighbors_list:
        ...
        for i in range(n_repeats):
            ...
            t1 = timeit.default_timer()
            labels = utils.run_leiden(weighted_graph, resolution=1.0) # default resolution = 1.0
            t2 = timeit.default_timer()
